...
<div style="background-color: white; margin-left: 80px; margin-right: 80px; height: 800px;">
	...
	<h1 style="color: red; margin-left: 50px;">Receptionist Login Page</h1><br /><br />
	<table>
	<tr>
...
$email = $_POST['email'];
 ...
 $result = mysqli_query($conn,"select *from reception where email = '$email' and password = '$password'")or die("failed to query databases".mysqli_error());
      
      
       if ( mysqli_num_rows($result)>0) {
       		$run=mysqli_fetch_array($result);
              echo "<script>window.open('transaction.php?rid=$run[0]','_self')</script>";
       }
          else
          {
